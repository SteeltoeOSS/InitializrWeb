...
describe('getListValues', () => {
  ...
  // it('parse correctly the list of Steeltoe versions', () => {
  //   const json = { ...MockClient }
  //   const listsValues = getLists(json)
  //   const listBootVersions = get(listsValues, 'steeltoe')
  //   const mockBootVersions = get(MockClient, 'steeltoeVersion.values')
...
describe('getProject', () => {
  ...
  it('should call the right service with the right parameters (no dependency)', () => {
    ...
    expect(fetch.mock.calls.length).toEqual(1)
    expect(fetch.mock.calls[0][0]).toEqual(
      'http://demo/starter.zip?name=foo8&namespace=&description=foo9&steeltoeVersion=&dotNetFramework=&language=foo2&packaging=&baseDir=foo8'
    )
  })
})
